...
async function handleBuy(product, durationCode, buyBtn, buyBtnText, statusEl) {
  ...
  try {
    ...
    // (например, кейсовый промокод уже использован), сбрасываем состояние
    // промокода на клиенте полностью: кнопка "Применено" и показанная
    // скидка иначе продолжали бы противоречить реальной сумме инвойса,
    // которую сервер уже посчитал без скидки.
    if (checkoutPromoCode && invoiceData.promo_invalid) {
      ...
      setPromoStatus("Промокод уже использован или недействителен, покупка по полной цене", "error");
    }
