...
function getFinalPrice(basePrice, discountPercent) {
  ...
  // Math.floor — специально, а не Math.round: сервер (bot.py, apply_promo)
  // считает финальную цену через math.floor. Если здесь округлять иначе,
  // цена, которую видит пользователь на экране, может не совпасть с той,
  // что реально спишется при оплате — это и была причина, по которой
  // маленькие скидки (например 5%) визуально "не работали".
...
function updateBuyButtonLabel(buyBtnText, product, durationCode) {
  ...
  // Показываем зачёркнутую полную цену и рядом с ней — новую цену со
  // скидкой, только когда скидка реально её снижает (актуально для
  // оплаты звёздами; для NFT/RU карты кнопка "Купить" не используется).
  if (checkoutDiscountPercent > 0 && finalPrice < duration.price && checkoutPaymentMethod === "stars") {
    checkoutOldPrice.textContent = `${duration.price} ★`;
    checkoutOldPrice.hidden = false;
    checkoutNewPrice.textContent = `${finalPrice} ★`;
    checkoutNewPrice.hidden = false;
  } else {
    checkoutOldPrice.hidden = true;
    checkoutNewPrice.hidden = true;
  }
...
function applyDurationPrice(priceEl, duration) {
  ...
  if (checkoutPaymentMethod === "nft") {
    ...
    // Промокод (в т.ч. любая процентная скидка, включая 5%) применяется
    // одинаково ко всем тарифам сразу — считаем финальную цену и, если
    // она реально ниже полной, показываем полную цену зачёркнутой рядом
    // с новой ценой со скидкой. getFinalPrice() округляет так же, как
    // сервер (math.floor), поэтому то, что видит пользователь, всегда
    // совпадает с тем, что реально спишется.
    const finalPrice = getFinalPrice(duration.price, checkoutDiscountPercent);

    if (checkoutDiscountPercent > 0 && finalPrice < duration.price) {
      priceEl.classList.add("duration-option-price--discounted");
      priceEl.innerHTML =
        `<span class="duration-option-price-old">${duration.price} ${STAR_ICON_SVG}</span>` +
        `<span class="duration-option-price-new">${finalPrice} ${STAR_ICON_SVG}</span>`;
    } else {
      priceEl.classList.remove("duration-option-price--discounted");
...
// вызывается при смене способа оплаты и при применении/снятии промокода,
// чтобы цены (включая зачёркнутую старую + новую со скидкой) сразу
// обновились без необходимости заново открывать экран оформления.
...
const checkoutNewPrice = document.getElementById("checkoutNewPrice");
...
function setPaymentMethod(method) {
  ...
  refreshDurationPrices();
  // На всякий случай пересобираем и цену на самой кнопке "Купить" — иначе
  // при переключении Stars → RU карта/NFT → обратно на Stars зачёркнутая
  // цена и цена со скидкой на кнопке могли остаться от предыдущего выбора.
  if (checkoutProduct) {
